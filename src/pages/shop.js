...
const readTags = (p) => Array.isArray(p?.tags)
  ? p.tags
...
const filterAndSort = (items, { category, brand, q, priceMin, priceMax }, sortKey) => {
  ...
  let out = (items || []).filter(p => {
    ...
    if (qn) {
      const haystack = [
        p?.name, p?.title, p?.subtitle, p?.description, c, b, ...readTags(p)
  ...
  switch (sortKey) {
    case 'price-asc': out.sort((a,b)=>readPrice(a)-readPrice(b)); break;
    case 'price-desc': out.sort((a,b)=>readPrice(b)-readPrice(a)); break;
    case 'name-asc': out.sort((a,b)=>(a?.name||a?.title||'').localeCompare(b?.name||b?.title||'','es')); break;
    case 'name-desc': out.sort((a,b)=>(b?.name||b?.title||'').localeCompare(a?.name||a?.title||'','es')); break;
    default: break; // relevance = sin cambios
  }
  return out;
};
...
const ShopPage = () => {
  ...
  const filtered = useMemo(() => {
    return filterAndSort(
      products,
      { category, brand, q, priceMin, priceMax },
      sort
    );
    ...
    <Layout>
      <div className={styles.root}>
        ...
        <Container size={'large'} spacing={'min'}>
          ...
          <div className={styles.metaContainer}>
            ...
            <div className={styles.controllerContainer}>
              ...
              <div className={`${styles.iconContainer} ${styles.sortContainer}`}>
                <label htmlFor="sort" style={{ marginRight: 8 }}>Ordenar por</label>
                <select
                  id="sort"
                  value={sort}
          ...
          {showFilter && (
            <div className={styles.filterPanel || undefined} style={{ padding: 16, border: '1px solid #eee', borderRadius: 12, marginBottom: 16 }}>
              <div style={{ display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))' }}>
                <div>
                  <label style={{ display: 'block', marginBottom: 6 }}>Categoría</label>
                  <select value={category} onChange={(e)=>setCategory(e.target.value)} style={{ width: '100%' }}>
                    <option value="">Todas</option>
                    {available.categories.map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                </div>
                <div>
                  ...
                  <select value={brand} onChange={(e)=>setBrand(e.target.value)} style={{ width: '100%' }}>
                    <option value="">Todas</option>
                    {available.brands.map(b => <option key={b} value={b}>{b}</option>)}
                  </select>
                </div>
                <div>
                  <label style={{ display: 'block', marginBottom: 6 }}>Buscar</label>
                  <input value={q} onChange={(e)=>setQ(e.target.value)} placeholder="resina, adhesivo, etc." style={{ width: '100%' }} />
                </div>
                <div>
                  <label style={{ display: 'block', marginBottom: 6 }}>Precio mínimo</label>
                  <input type="number" value={priceMin} min={available.priceRange[0]} max={priceMax} onChange={(e)=>setPriceMin(Number(e.target.value)||0)} style={{ width: '100%' }} />
                </div>
                <div>
                  <label style={{ display: 'block', marginBottom: 6 }}>Precio máximo</label>
                  <input type="number" value={priceMax} min={priceMin} max={available.priceRange[1]} onChange={(e)=>setPriceMax(Number(e.target.value)||0)} style={{ width: '100%' }} />
                </div>
              </div>

              <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
                <Button level="secondary" onClick={()=>setShowFilter(false)}>Aplicar</Button>
                <Button level="tertiary" onClick={clearAll}>Limpiar filtros</Button>
              </div>
            </div>
          )}
          ...
          <div className={styles.loadMoreContainer}>
            <span>{Math.min(visibleCount, total)} de {total}</span>
            <Button
              fullWidth
              level={'secondary'}
